...
これは(PCあるいはマザーボード)メーカーによって様々ですが、BIOS/UEFIに入るキーとして自作PCつまりは多くのパーツとして販売しているマザーボードでは`Delete(DEL)`キー、メーカーPC(Dell、Acer、Lenovo等)の場合は`F2`キー、HPは`F10`キーなどがよく使用されます。
中には`F12`キーで起動時のブートメニューの呼び出しなどから入ることもあるそうです。
...
ブートマネージャーは、たいていGrubで問題ないです。その時起動できるものでも大丈夫と思います。CachyOSで言うと、ISOから起動するものは**ライブ環境**なのでその段階ではPCへの変更はありません。
