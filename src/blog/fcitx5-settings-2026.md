...
env=GTK_IM_MODULES,wayland;fcitx
env=QT_IM_MODULES,wayland;fcitx
```

MangoWMでは上記のような書き方ができます。
...
こういう環境変数を書いておく方が良い場合があります。
Hyprlandでは`hyprland.lua`の上の方に**何かコードブロックで囲む必要無く** 、上記のコードだけを書きます。
...
そもそもシステムファイルである`/etc/environment`を書き換える方法がわからないと言う人もいたかも知れません。そういう人にこれまでは**必要なことだから**と書き換えを強要していたわけです。
