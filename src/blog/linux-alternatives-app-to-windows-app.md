...
Windowsは確かに<u>一般人が乗るセダン</u>としては優秀ですが、世界最速の車・最先端の自動運転車・宇宙に行ってる車は全部Linux((Tesla、Boston Dynamics、NASAのローバー、JAXAはやぶさ2))です。つまりLinuxは**公道を走ってるように見えないだけで、実はもうF1マシン＋宇宙船レベル**なのです。
...
DaVinci Resolveは最近のアップデートでAI対応が強化されました、AI自体の機能はクロスプラットフォームに実装されているので機能の有無で大きな差がでているわけではありませんが、Linux版では「レンダリング・処理速度が大幅に向上した」、「AI機能の実際の体感速度が速い」、「OS自体が軽量なので安定している」などということが言われることが増えました。
特に同じPC・同じGPUでLinux版の方がWindowsより3～4倍速くレンダリングが完了するという報告があったと言います(DaVinci Resolve 20以降)。信じがたい報告ですが話半分に聞いても1.5～2倍速いとすればこれは特筆すべきことになります。
要因としては、Windowsのバックグウンドプロセスやドライバーのオーバーヘッドが大きく、CUDAやOpenGLを使った重いAI処理・エクスポートでLinuxの方が明確に有利になるケースが増えています。
他の事柄も全て同じ要因が絡んでいます。つまりはWindowsが重くなりすぎたということです。

結果的には、「<u>AIをフルに活かそうとするとLinuxの方が明らかに速く・快適</u>」という状況が出来上がってきています。特にハイエンドGPU（RTX 40/50シリーズなど）を使っていて、レンダリングやAI処理を多用する人にとっては、Linux + DaVinci Resolve Studioが現時点で最もパフォーマンスを引き出せる組み合わせの一つになっています。ただし導入時の設定などがWindowsほど簡単ではないことが問題点になりますが、それを許容できる人であればLinuxを使う理由には十分なり得ます。
...
純粋にテキストだけを編集する場合は、これもディストリビューションやデスクトップ環境によって独自に提供されていたりします。例えばGnomeであればテキストエディター(gnome-text-editor)と言うのもあり、他にも[gedit](https://gedit-text-editor.org/)というのもあります。
...
> ここに繋げるわけではありませんが、以前Linuxのディストリビューションにはどういった物があるかという記事で、各ディストリビューションの説明にパッケージマネージャーを書いてあります。これらを参考にしてみて下さい。
> [Linuxの系統別 解説](/blog/explanation-of-linux-systems/)
...
等から来ることが多く、**全体のユーザーの中では少数派**ですが、<u>目立つ</u>という事が言われています。

日本車しかいじったことがなく日本車に合う道具しか持っていない人が、外車を購入してネジの規格が違うことで、(面倒くさい)これはダメだと思うような、あるいは大工でなんでも作れる人が外国に移住してノコギリの違い、カンナの違いに戸惑うようなそういったものが総じて「使えない」と言われている感じです。

例えばLinux MintをDistroWatchのページヒットランクを見て選んだ場合、<u>Windowsと大きく変化がない</u>し<u>退屈</u>じゃないかと言う感想があってもそれはしょうがありません。
Windowsから移ってもなるべく大きな違和感を感じないように作ってあるのですから。

<u>変化がないし退屈じゃないか</u>と思うあなたにもマッチする、あるいはまだ試していないディストリビューションはあるはずです。
けどもそれはただのOSの話です。性能が良い、機能充実、あまり考える必要がないと言った、人を堕落させるような方向で進んできたのがWindowsでありMacOSです。使う人がそれであればまだしも、作り手もAIに頼ったり、本当の意味での便利さを履き違えることはありますよね？
無駄に重いのを貧乏だからメモリも買えないのか？もっと良いグラボやSSDに変えろ、いっそのこと新しいPCにしたらいいじゃないかというのは、考えを停止した人の戯れ言です。
なので、自分から勇気を出して試してほしいのです。人の意見に左右されずに実際に自分が使ってみてどうかを試してほしいのです。<u>完全移行する必要はありません</u>。古いPCにLinuxを入れてみる。それがまだ未経験な内はLinuxを評価するスタートラインにも立ててないのです。

## まとめ

他にも色々とありますが、よくWindowsを使用している人が言う「Linuxにはソフトがない」と言うのは間違っていて、むしろ細かいものはWindows並にあるとも言えますが、主語に「<u>自分が使っているソフトは</u>」と言われると「無い」と言うのはその通りかも知れません。
...
WinBoatは、まだベータ版ではあるもののソフトがOSの垣根を超えて使用できるようになる試みです。

しかし、ここまでを見てきてもらったら、**ソフトが不足していると言うわけではない**というのは理解してもらえるでしょうから、後はどこにそれがあり、自分の環境で使えるのか問題だけがあって、それがクリアできたら普段遣いで困ることはありません。設定が難しいかも知れませんが、できるかできないかで言えば**できる**と言えるだろうと思います。
...
広告のためのテレメトリーもなく、**リソースの消費は圧倒的に少ない**Linuxですから、(問題が起こっておらず正常に処理できているのであれば)Windowsと同じソフトを使用した場合に、より快適な体験があるだろうと思います。
